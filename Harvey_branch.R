...
# 
# #County data
nycounties <- readOGR("virus_data/gz_2010_us_050_00_20m.geojson")
# head(nycounties)
# nycounties
# 
# class(nycounties)
...
nycounties_clear <- na.omit(nycounties@data[nycounties$STATE=="36",])
# nycounties$COVID_Case_over_CENSUSAREA <- as.matrix(floor(nycounties@data$COVID_Case / nycounties@data$CENSUSAREA))
...
server <- function(input, output, session) {
  ...
  observe({
    ...
    output$barPlot <- renderPlot({
      barplot(nycounties_clear$CENSUSAREA)})
  })
